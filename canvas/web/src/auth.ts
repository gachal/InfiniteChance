/**
 * canvas-web 的会话状态:本地保存的 JWT、当前身份、初始化进度。
 * 与 admin-web 同一 JWT 体系 —— 后端共享 admin_accounts 表与签名密钥,
 * 网关侧创建的账号在画布侧直接登录,无需二次注册。
...
const TOKEN_KEY = 'infinitechance.canvas.token'
...
export function authErrorMessage(e: unknown): string {
  return e instanceof ApiError ? e.message : '无法连接画布服务,请确认后端已启动'
}
